  ...
  private setupRoutes(): void {
      ...
      try {
        ...
        if (
          ...
          return res.status(400).json({
            success: false,
            error:
              "Invalid test case format. Required fields: id, prompt, serverConfigs",
          });
        }

        this.logger.info(
          `🧪 Starting test execution for: ${testCase.name || testCase.id}`,
        );
    ...
    this.app.post("/api/test/run-batch", async (req, res) => {
      try {
        ...
        this.logger.info(
          `🧪 Starting batch test execution for ${testCases.length} tests`,
        );
      ...
      try {
        ...
        if (!testCase.name || !testCase.prompt || !testCase.serverConfigs) {
          return res.status(400).json({
            success: false,
            error:
              "Invalid test case format. Required fields: name, prompt, serverConfigs",
          });
        }
  ...
  async start(
    ...
    return new Promise((resolve, reject) => {
      this.server = this.app.listen(this.config.port, this.config.host, () => {
        this.logger.info(
          `🧪 Test server listening on ${this.config.host}:${this.config.port}`,
        );
        resolve();
      });
