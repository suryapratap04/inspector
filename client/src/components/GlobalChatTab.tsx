...
const validateSendConditions = (
  ...
  return {
    ...
    canSend: input.trim() && mcpAgent && hasAnyApiKey && !loading,
  };
};
...
}> = ({ onSuggestionClick, selectedProvider, connectedToolsCount }) => {
  const suggestions = [
  ...
  return (
    <div className="flex items-center justify-center h-full p-8">
      <div className="text-center max-w-md space-y-6">
        ...
        <div className="space-y-2">
          <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">
            Global Chat with {getProviderDisplayName(selectedProvider)}
          </h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Chat with access to tools from all connected servers
          </p>
          <p className="text-xs text-slate-400 dark:text-slate-500">
            {connectedToolsCount} connected tools available
          </p>
...
const GlobalChatTab: React.FC<GlobalChatTabProps> = ({ mcpAgent }) => {
  ...
  // Get connected servers count for display
  const connectedServersCount = React.useMemo(() => {
  ...
  const processUserQuery = async (
    ...
    if (!mcpAgent) {
      throw new Error(
        "Global chat functionality is not available. Please ensure you have connected servers and a valid API key.",
      );
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-semibold text-slate-900 dark:text-slate-100">
                  Global Chat - {getProviderDisplayName(selectedProvider)}
                </h1>
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {hasAnyApiKey 
                    ? `${connectedServersCount} connected servers • ${tools.length} tools available`
                    : "API key required"
                  }
                </p>
              </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {!hasAnyApiKey ? (
          ...
          <EmptyChatsState
            ...
            connectedToolsCount={tools.length}
          />
        ) : (
          <div className="py-2">
            ...
            {hasAnyApiKey && !loading && (
              <div className="text-xs text-slate-400 dark:text-slate-500 mt-3 px-1">
                <span className="hidden sm:inline">
                  Press Enter to send • Shift+Enter for new line • Tools from {connectedServersCount} connected servers
                </span>
              </div>
            )}
          </form>
