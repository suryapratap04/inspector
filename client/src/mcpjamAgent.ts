...
import { Tool, Resource, Prompt, ServerCapabilities } from "@modelcontextprotocol/sdk/types.js";
...
export class MCPJamAgent {
    ...
    private onPendingRequest?: (request: CreateMessageRequest, resolve: (result: CreateMessageResult) => void, reject: (error: Error) => void) => void;
    ...
    async connectToAllServers(): Promise<void> {
        const connectionPromises = Object.keys(this.serverConfigs).map(serverName => 
    ...
    async disconnectFromAllServers(): Promise<void> {
        const disconnectionPromises = Array.from(this.mcpClientsById.entries()).map(async ([name, client]) => {
            try {
                await client.disconnect();
            } catch (error) {
    ...
    async getAllResources(): Promise<{ serverName: string; resources: Resource[] }[]> {
        ...
        for (const [serverName, serverConfig] of Object.entries(this.serverConfigs)) {
            try {
                ...
                console.error(`Failed to get resources from server ${serverName}:`, error);
                allServerResources.push({ 
                    serverName, 
                    resources: [] 
                });
    ...
    async callToolOnServer(serverName: string, toolName: string, params: Record<string, unknown>) {
    ...
    async getPromptFromServer(serverName: string, name: string, args: Record<string, string> = {}) {
        const client = await this.getConnectedClientForServer(serverName);
    ...
    updateCredentials(bearerToken?: string, headerName?: string, claudeApiKey?: string) {
        this.bearerToken = bearerToken;
        this.headerName = headerName;
        this.claudeApiKey = claudeApiKey;
    ...
    // Get aggregated request history from all servers
    getAllRequestHistory(): { serverName: string; history: { request: string; response?: string }[] }[] {
        return Array.from(this.mcpClientsById.entries()).map(([serverName, client]) => ({
            serverName,
            history: client.requestHistory || []
        }));
    }

    // Get aggregated connection status
    getOverallConnectionStatus(): "connected" | "partial" | "disconnected" | "error" {
