...
import { MCPClient, ExtendedMcpClient } from "../mcpClient";
...
export function useConnection({
  ...
  const [mcpClient, setMcpClient] = useState<ExtendedMcpClient | undefined>(client);
  ...
  const updateApiKey = (newApiKey: string) => {
    if (mcpClient && mcpClient.anthropic) {
      // Import Anthropic dynamically to avoid import issues
  ...
  const connect = async (_e?: unknown, retryCount: number = 0, mcpClient?: MCPClient) => {
    let client;
    if (mcpClient){
      client = mcpClient
    }
    else {
      client = new MCPClient(claudeApiKey);
    }
